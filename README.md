...
### Probabilistic Early Recomputation (PER)
- 개념
  - PER는 캐시 만료 시점이 오기 전에, 확률적으로 캐시를 조기에 갱신하는 전략
  - 모든 요청이 아니라 일부 요청만 갱신 책임을 갖도록 하여 Cache Stampede를 방지
  - 만료 시간이 가까워질수록, 그리고 캐시 갱신 비용이 클수록 갱신 확률이 증가
- 기본 아이디어
  - 캐시가 아직 유효하더라도, 특정 요청은 확률적으로 캐시를 재계산
  - 나머지 요청들은 기존 캐시 데이터를 그대로 사용
  - 결과적으로 캐시 만료 시점에 Data Source로 요청이 몰리는 현상을 완화
- 요청 흐름
  - 여러 요청이 동일한 Key로 Cache를 조회
  - 캐시에 데이터가 존재하므로 기본적으로 캐시 히트
  - 이 중 일부 요청만 확률적으로 캐시 갱신 책임을 획득
  - 선택된 요청만 Data Source를 조회하여 캐시를 갱신
  - 이후 요청들은 갱신된 캐시 데이터를 즉시 사용
- 왜 Cache Stampede를 막을 수 있는가
  - 캐시 만료 이후가 아니라, 만료 이전에 미리 갱신이 수행됨
  - 모든 요청이 동시에 Data Source로 전파되지 않음
  - 확률 기반이므로 갱신 작업이 자연스럽게 분산됨
- 확률 계산에 사용되는 요소
  - 만료 시간(expiry)
    - 현재 시간이 만료 시점에 가까울수록 갱신 확률 증가
  - 재계산 시간(delta)
    - 캐시 재계산이 오래 걸릴수록 갱신 확률 증가
  - 난수(rand)
    - 요청마다 서로 다른 확률적 판단을 수행
- 핵심 알고리즘 개요
  - 캐시에서 (value, delta, expiry)를 조회
  - 아래 조건을 만족하면 캐시 갱신 수행
- 조건식 개념
  - (현재 시간 - delta × beta × log(난수)) ≥ 만료 시점
  - log(rand)는 항상 0 이하의 값을 가지므로 확률적 변동성 생성
  - delta가 크거나, 현재 시간이 expiry에 가까울수록 조건을 만족할 가능성 증가
- 알고리즘 흐름 요약
  - 캐시에 값이 없으면 즉시 재계산
  - 캐시에 값이 있더라도 확률적으로 재계산 여부 판단
  - 재계산 시:
    - Data Source 조회
    - 재계산 소요 시간(delta) 측정
    - 캐시 값과 delta를 함께 갱신
...
- PER의 특징 정리
  - 캐시 만료 시점에 의존하지 않음
  - 락이나 강제 동기화 없이 Stampede 완화 가능
  - 갱신 비용이 큰 데이터일수록 효과가 큼
  - 수학적/확률적 개념이 포함되어 구현 난이도는 다소 높음
- 적합한 사용 사례
  - 재계산 비용이 높은 데이터
  - 트래픽이 많은 단일 Key
  - 만료 시점에 부하가 집중되는 시스템
